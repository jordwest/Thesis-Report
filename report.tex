\newcommand{\SoftName}{GMemorise}
\newcommand{\HRule}{\rule{\linewidth}{0.5mm}}
\makeatletter
\g@addto@macro\@floatboxreset\centering
\makeatother

\documentclass[a4paper]{article}
\usepackage[margin=3cm]{geometry}
\usepackage[pdftex]{graphicx}
\usepackage{float}
\usepackage{hyperref}
\usepackage[parfill]{parskip}
\usepackage[justification=centering]{caption}
\usepackage{url}
\usepackage{biblatex}
\bibliography{thesis.bib}

\title{Predicting Responses to Spaced Repetition Flash Cards with Machine Learning Techniques}
\date{5 November, 2012}
\author{Jordan West}

\begin{document}

% \maketitle
\input{titlepage.tex}

\tableofcontents
\newpage

\input{introduction.tex}
\input{background.tex}
\input{goals.tex}
\input{method.tex}
\input{results.tex}
\input{discussion.tex}

\section{Acknowledgments}
This thesis would not have been possible without the support of my
supervisor Dr. Mark Schulz whose input and guidance
has been invaluable for the project.

I would like to thank Dr Yuriko Nagata for her assistance with this and for allowing
me to introduce the software to her students.

Dr Michael Harrington of the School of Languages and Comparative Cultural Studies
for his advice with the online learning software, particularly the suggestion
to record the time students take to review individual words.

I would also like to thank the UQ CEIT team for their help with this project -
particularly Alan Cody and Phil Long.

Last but not least I want to thank my friends for their encouragement and
for putting up with me throughout the project, and Mum and Dad for supporting me
% TODO Finish Acknowledgements

%\bibliographystyle{plain}
%\bibliography{thesis}
\printbibliography

\appendix
\section{Participant Information Sheet}
\label{appendix_participant_info_sheet}
\fbox{\includegraphics[page=1,width=15cm]{appendix/participant_info_sheet.pdf}}
\section{Participant Consent Form}
\label{appendix_participant_consent_form}
\fbox{\includegraphics[page=1,width=15cm]{appendix/participant_consent_form.pdf}}
\section{Original Wireframes}
\label{appendix_wireframes}
\fbox{\includegraphics[page=1,width=15cm]{appendix/wireframes.pdf}}\newpage
\fbox{\includegraphics[page=2,width=15cm]{appendix/wireframes.pdf}}
\fbox{\includegraphics[page=3,width=15cm]{appendix/wireframes.pdf}}
\fbox{\includegraphics[page=4,width=15cm]{appendix/wireframes.pdf}}
\fbox{\includegraphics[page=5,width=15cm]{appendix/wireframes.pdf}}
\fbox{\includegraphics[page=6,width=15cm]{appendix/wireframes.pdf}}

\end{document}
